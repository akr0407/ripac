...
export default defineEventHandler(async (event) => {
    try {
        ...
        const [result] = await db.insert(comments).values({ registrationId, commentText }).returning();

        return { success: true, data: result };
    } catch (error) {
        if (error instanceof z.ZodError) throw createError({ statusCode: 400, statusMessage: 'Validation failed' });
        if ((error as any).statusCode) throw error;
        throw createError({ statusCode: 500, statusMessage: 'Failed to save comment' });
    }
});
